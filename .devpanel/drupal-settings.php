...
if (getenv('MAPS_TOKEN')) {
  $settings['usc_court_finder.google_maps_places.api_key'] = getenv('MAPS_TOKEN');
}

if (getenv('BROWSEALOUD_SRI_VERSION')) {
  $settings['browsealoud.sri_version'] = getenv('BROWSEALOUD_SRI_VERSION');
}

if (getenv('BROWSEALOUD_SRI_DIGEST')) {
  $settings['browsealoud.sri_digest'] = getenv('BROWSEALOUD_SRI_DIGEST');
}
...
switch (getenv('GIT_BRANCH')) {
  ...
  case "main";
  	...
  	$config['environment_indicator.indicator']['bg_color'] = '#01417F';
    break;
}
